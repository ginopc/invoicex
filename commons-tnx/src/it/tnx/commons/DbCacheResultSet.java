 ...
 * @author mceccarelli
 */
public class DbCacheResultSet implements ResultSet  {
    ArrayList<Map> rn = new ArrayList();    //records per nome
    ArrayList<Object[]> ri = new ArrayList();    //record per colonna
    int currentrow = -1;

    public DbCacheResultSet(Connection conn, String sql, ResultSet r) {
        try {
            fillList(conn, sql, r);    
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public DbCacheResultSet(List<Map> list) {
        try {
            fillList(list);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void fillList(List<Map> list) {
        if (list.size() <= 0) return;
        Map rtemplate = list.get(0);
        int columns = rtemplate.keySet().size();
        Set columnset = rtemplate.keySet();
        Object[] columnsarray = columnset.toArray();
        for (Map r : list) {
            Object[] row = new Object[columns];
            HashMap map = new HashMap();
            for (int i = 0; i < columns; i++) {
                Object o = r.get(columnsarray[i]);
                row[i] = o;
                map.put(columnsarray[i], o);
            }
            ri.add(row);
            rn.add(map);
        }
    ...
    public String getString(String columnLabel) throws SQLException {
        return cu.toString(rn.get(currentrow).get(columnLabel));
    }
    ...
    public int getInt(String columnLabel) throws SQLException {
        return cu.toInteger(rn.get(currentrow).get(columnLabel));
    }
    ...
    public double getDouble(String columnLabel) throws SQLException {
        return cu.toDouble(rn.get(currentrow).get(columnLabel));
    }

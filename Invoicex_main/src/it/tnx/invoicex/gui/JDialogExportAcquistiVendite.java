...
public class JDialogExportAcquistiVendite extends javax.swing.JDialog {
    ...
    public Integer fornitore = -1;
    public ReadyTec rt = null;
    ...
    public JDialogExportAcquistiVendite(java.awt.Frame parent, boolean modal) {
        ...
        //rimasta a metà modifica toys
        filtroTipo.setVisible(false);
...
//
    ...
    private void initComponents() {
        ...
        filtroTipo.addActionListener(formListener);
            ...
            .add(layout.createSequentialGroup()
                ...
                .add(layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
                    ...
                    .add(layout.createSequentialGroup()
                        ...
                        .add(layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
                            .add(layout.createSequentialGroup()
                                .add(filtroTipo, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
    ...
    private class FormListener implements java.awt.event.ActionListener, java.awt.event.FocusListener, java.awt.event.ItemListener {
        ...
        public void actionPerformed(java.awt.event.ActionEvent evt) {
            ...
            else if (evt.getSource() == filtroTipo) {
                JDialogExportAcquistiVendite.this.filtroTipoActionPerformed(evt);
            }
        }
            ...
            if (lista_fatture_v.contains(this.tipo.getSelectedItem())) {
                if (!this.raggruppa.isSelected()) {
                    if (cheShowRef.isSelected()) {
                        headerWidth = new int[]{5, 8, 8, 12, 5, 4, 4, 12, 6, 6, 6, 3, 4, 6, 6, 3, 3, 3, 3, 3, 6, 6, 6};
                    } else {
                        headerWidth = new int[]{5, 8, 8, 15, 5, 4, 4, 15, 6, 6, 3, 4, 6, 6, 3, 3, 3, 3, 3, 6, 6, 6};
                    }
                    sql = sql + " r.codice_articolo, ";

                    sql = sql + " cat.categoria, ";
                    sql = sql + " sotcat.sottocategoria as sotto_categoria, ";

                    sql = sql + " r.descrizione, ";
                    if (main.pluginAttivi.contains("pluginToysforyou")) {
                        sql = sql + " l.ita as produttore, ";
                        if (cheShowRef.isSelected()) {
                            headerWidth = new int[]{5, 8, 8, 12, 10, 5, 4, 4, 12, 6, 6, 6, 3, 4, 6, 6, 3, 3, 3, 3, 3, 6, 6, 6};
                        } else {
                            headerWidth = new int[]{5, 8, 8, 15, 10, 5, 4, 4, 15, 6, 6, 3, 4, 6, 6, 3, 3, 3, 3, 3, 6, 6, 6};
                        }
                    }
...
//                    sql = sql + " , sum(round(calcola_importo_netto(r.prezzo * r.quantita, r.sconto1, r.sconto2, t.sconto1, t.sconto2, t.sconto3),2) + round((round(calcola_importo_netto(r.prezzo * r.quantita, r.sconto1, r.sconto2, t.sconto1, t.sconto2, t.sconto3),2) / 100 * i.percentuale),2))  + IFNULL(t.spese_trasporto,0) + IFNULL(t.spese_incasso,0) as 'importo totale' ";
                ...
                sql += " left join codici_iva i on iva = i.codice \n";

                sql += " left join articoli a on r.codice_articolo = a.codice \n";
                sql += " left join categorie_articoli cat on a.categoria = cat.id \n";
                sql += " left join sottocategorie_articoli sotcat on a.sottocategoria = sotcat.id \n";
...
//                    sql = sql + " , sum(round(calcola_importo_netto(r.prezzo * r.quantita, r.sconto1, r.sconto2, t.sconto1, t.sconto2, t.sconto3),2) + round((round(calcola_importo_netto(r.prezzo * r.quantita, r.sconto1, r.sconto2, t.sconto1, t.sconto2, t.sconto3),2) / 100 * i.percentuale),2))  + IFNULL(t.spese_trasporto,0) + IFNULL(t.spese_incasso,0) as 'importo totale' ";
            ...
            if (this.comCliente.getSelectedIndex() > 0) {
                if (this.tipo.getSelectedItem().toString().toLowerCase().indexOf("vendita") >= 0) {
                    sql = sql + " and t.cliente = '" + this.comCliente.getSelectedKey() + "'";
                } else {
                    sql = sql + " and t.fornitore = '" + this.comCliente.getSelectedKey() + "'";
                }
            }
            if ((this.comFornitore.getSelectedIndex() > 0)
                    && (this.tipo.getSelectedItem().toString().toLowerCase().indexOf("acquisto") >= 0)) {
                sql = sql + " and t.fornitore = '" + this.comFornitore.getSelectedKey() + "'";
            }
//            if (main.pluginAttivi.contains("pluginToysforyou")) {
...
//            }
            ...
            String tipoDocumento = String.valueOf(this.tipo.getSelectedItem());
            if (tipoDocumento.contains("Vendita") && this.fornitore > 0) {

                String subQuery = "SELECT a.codice\n"
                        + "FROM articoli a \n"
                        + "LEFT JOIN produttori p ON a.produttore_id = p.id \n"
                        + "LEFT JOIN fornitori_produttori fp ON fp.produttore_id = p.id \n"
                        + "WHERE fp.cliente_id = " + Db.pc(fornitore, Types.INTEGER);

                sql = sql + " and a.codice IN (" + subQuery + ") ";
            }
...
//non c'è più funzione calcola_importo_netto                
...
//                }
    ...
    private void comFornitoreItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_comFornitoreItemStateChanged
        if (evt.getStateChange() == ItemEvent.SELECTED) {
            InvoicexUtil.fireEvent(this, InvoicexEvent.TYPE_EXPORTAV_FORNITORE_POST_SELECTION);
        }
    ...
    private String getNomeTab() {
        ...
        if (lista_fatture_v.contains(this.tipo.getSelectedItem())) {
            return "test_fatt";
        } else if (tipo.getSelectedItem().equals("Acquisti (Fatture di Acquisto)")) {
            return "test_fatt_acquisto";
        } else if (tipo.getSelectedItem().equals("DDT di Vendita")) {
            return "test_ddt";
        } else if (tipo.getSelectedItem().equals("DDT di Acquisto")) {
            return "test_ddt_acquisto";
        } else if (tipo.getSelectedItem().equals("Preventivi/Ordini di Vendita")) {
            return "test_ddt_acquisto";
        } else if (tipo.getSelectedItem().equals("Preventivi/Ordini di Acquisto")) {
            return "test_ddt_acquisto";
        }
    ...
    public void exportFatSeq() {
        String query = "SELECT t.*, c.* , p.id_pagamento_teamsystem, p.tipo_effetto_teamsystem FROM "
                ...
                + "WHERE (tipo_fattura != " + dbFattura.TIPO_FATTURA_SCONTRINO + " and tipo_fattura != " + dbFattura.TIPO_FATTURA_PROFORMA + ")";
        if (this.comCliente.getSelectedIndex() > 0) {
        ...
        List<Map> list = null;
        try {
            list = DbUtils.getListMap(Db.getConn(), query);
        } catch (Exception e) {
            e.printStackTrace();
        }

        rt = new ReadyTec(list, this);
        rt.start();
    }
}

...
//                sito.setVisible(true);
    ...
    public static boolean scarica(String p) throws Exception {
        URL urlagg = new URL(main.baseurlserver + "/" + InvoicexUtil.getDownloadDir() + "/" + p);
...
//        int size = conn.getContentLength();
        ...
        GetMethod httpGet = new GetMethod(urlagg.toString());
        try {
            httpclient.executeMethod(httpGet);
            
            int retcode = httpGet.getStatusLine().getStatusCode();
            if (retcode != 200) {
                System.out.println("scaricap: errore retcode:" + retcode);
                return false;
            }
    ...
    private void d3(int i) {
        ...
        try {
            ...
            url = new URL(main.baseurlserver + "/connect3.php?r=" + i + "&i=" + id);
...
//            }
            ...
            String content = HttpUtils.getUrlToStringUTF8(url.toString());
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
...
//                    model = new TableModelPlugins(service.getPlugins2(false));
                    ...
                    model = new TableModelPlugins(InvoicexUtil.getPlugins(false, false));
                    ...
                    ex.printStackTrace();
                    JOptionPane.showMessageDialog(JDialogPlugins.this, "Errore: " + ex.toString(), "Errore", JOptionPane.ERROR_MESSAGE);
                    dispose();
                }
            ...
            public void finished() {
                ...
                synchronized(aggiorna) {
                    if (model != null) {
                        ...
                        tcm.getColumn(2).setHeaderRenderer(new PluginsHeaderRenderer2(tabPlugins.getTableHeader().getDefaultRenderer()));
                        tcm.getColumn(2).setCellRenderer(new PluginsRenderer2());

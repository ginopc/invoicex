...
public class JRDSDdt extends JRDSBase
    ...
    public JRDSDdt(Connection conn, String serie, int numero, int anno, boolean perEmail, boolean acquisto, Integer id) {
        ...
        sql = "select t.*,r.*,clie_forn.*, clie_forn_dest.*, agenti.*, articoli.* "
                + " , clie_forn.codice as codice_cliente2 " + (acquisto ? ", '' as cliente, '' as cliente_destinazione " : "")
                + " , l.codice as listino \n"
...
//!!! eventualmente prendere da deposito        
        try {
            ...
            rDatiAzienda.close();
            sDatiAzienda.close();
            //controllo che non ci sia da riprendere i dati dal deposito
        } catch (Exception err) {
            err.printStackTrace();
        }
...
//campi direttametne dal db
    ...
    private Object getImg(boolean isLogo, boolean isInputStream) {
        return JRDSInvoice.getImg(isLogo, isInputStream, serie, numero, anno, perEmail, acquisto, acquisto ? "test_ddt_acquisto" : "test_ddt", dep_id);
    }
}

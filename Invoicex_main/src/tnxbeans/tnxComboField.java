...
//            timFindC.start();
    ...
    public boolean dbTrovaKey2(Object chiave) {
        ...
        if (!lazy) {
            if (getItemCount() > 0 && getItemAt(0) instanceof KeyValuePair) {
                for (int i = 0; i < getItemCount(); i++) {
                    if (getItemAt(i) instanceof KeyValuePair) {
                        if (((KeyValuePair)getItemAt(i)).key.equals(chiave)) {
            ...
            for (int i = 0; i < this.dbItemsK.size(); i++) {
                ...
                } else if (String.valueOf(this.dbItemsK.get(i)).equalsIgnoreCase(String.valueOf(chiave))) {
                    found = true;
                }
                if (found) {
        ...
        temp.addMouseListener(new java.awt.event.MouseListener() {
            public void mousePressed(MouseEvent evt) {
                if (evt.isPopupTrigger()) {
                    ...
                    temp.show(tnxComboField.this, evt.getX(), evt.getY());
                }
            }
    ...
    void this_keyTyped(KeyEvent e) {
        ...
        } else if (this.dbNomeCampo != null && this.dbNomeCampo.length() > 0) {
...
//                                this.setSelectedIndex(0);
                    ...
                    } else if (valoreSelezionato != null && valoreSelezionato.length() > 0) {
                        this.dbTrovaKey(valoreSelezionato);
                    } else {
                        //                    this.setSelectedItem(null);
                        try {
...
//                                        tnxComboField.this.setSelectedIndex(0);
    ...
    public boolean dbOpenListArray(ArrayList<Object[]> list, final String valoreSelezionato) {
        this.isOpeningList = true;

        if (SwingUtilities.isEventDispatchThread()) {
            if (list.size() > 0 && list.get(0).length > 1) {
                this.contieneChiavi = true;
            }
            //righe
            for (Object[] rec : list) {

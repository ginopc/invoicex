    ...
    public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
        ...
        if (!substance) {
            if (super.getCellRenderer(row, column).getClass().getName().startsWith("java")
                    ...
                    || super.getCellRenderer(row, column).getClass().getName().startsWith("gestioneFatture")) {
...
//                                double d = (Double.valueOf(value.toString())).doubleValue();
    ...
    public HashMap getColumnsTitlesDisplayed() {
        if (columnsTitleDisplayed == null) {
            columnsTitleDisplayed = new HashMap();
        }
        return columnsTitleDisplayed;
    }

    public void setColumnsTitleDisplayed(HashMap columnsTitleDisplayed) {
        this.columnsTitleDisplayed = columnsTitleDisplayed;
    }

    public void calcolaColSizePerc() {
        if (columnsSizePerc != null) {
            if (columnsSizePercOrig == null || columnsSizePercOrig.size() != columnsSizePerc.size()) {
                columnsSizePercOrig = (Hashtable) columnsSizePerc.clone();
            }

            Set s = columnsSizePercOrig.keySet();
            ...
            while (iter.hasNext()) {
                ...
                try {
                    getColumn(col);
                    totale += (Double) columnsSizePercOrig.get(col);
                } catch (Exception e) {
                }
            }
            iter = s.iterator();
            columnsSizePerc.clear();
            while (iter.hasNext()) {
                String col = (String) iter.next();
                try {
//                    getColumn(col);
                    Double current = (Double) columnsSizePercOrig.get(col);
//                    System.out.println("col: " + col + " current = " + current);
                    columnsSizePerc.put(col, current * 100d / totale);
//                    System.out.println("columnsSizePerc.get " + col + " = " + columnsSizePerc.get(col));
                } catch (java.lang.IllegalArgumentException e) {
                    e.printStackTrace();
                }
            }
            System.out.println("columnsSizePerc = " + columnsSizePerc);
        }
    }

    private ArrayList memorizza_renderers() {
        if (getColumnModel() != null) {
            ArrayList old_renderers = new ArrayList();
            for (int i = 0; i < getColumnCount(); i++) {
                if (getColumnModel().getColumn(i).getCellRenderer() != null) {
                    old_renderers.add(getColumnModel().getColumn(i).getCellRenderer());
                } else {
                    old_renderers.add(null);
                }
            }
            return old_renderers;
        }
        return null;
    }
    ...
    private ArrayList memorizza_editors() {
        if (getColumnModel() != null) {
            ArrayList old_editors = new ArrayList();
            for (int i = 0; i < getColumnCount(); i++) {
                if (getColumnModel().getColumn(i).getCellEditor() != null) {
                    old_editors.add(getColumnModel().getColumn(i).getCellEditor());
                } else {
                    old_editors.add(null);
                }
            }
            return old_editors;
        }
    ...
    public String stampaTabella(String titolo, int[] headerWidth) {
        return stampaTabella(titolo, headerWidth, null, null);
    }

    public String stampaTabella(String titolo, int[] headerWidth, String piede) {
        return stampaTabella(titolo, headerWidth, piede, null);
    }

    public String stampaTabella(String titolo, int[] headerWidth, String piede, String intestazione_aggiuntiva) {
        return stampaTabella(titolo, headerWidth, piede, intestazione_aggiuntiva, null);
    }

    public String stampaTabella(String titolo, int[] headerWidth, String piede, String intestazione_aggiuntiva, MyCallable pre_int_tab) {
        return stampaTabella(titolo, headerWidth, piede, intestazione_aggiuntiva, pre_int_tab, null, null);
    }

    public String stampaTabella(String titolo, int[] headerWidth, String piede, String intestazione_aggiuntiva, MyCallable pre_int_tab, String nomeStampa, MyCallableProcessValue process_value) {
        ...
        Integer carattere = 0;
        if (nomeStampa != null) {
            formato = main.fileIni.getValue("pref", "param_stampa_" + nomeStampa + "_formato", "verticale");
            carattere = cu.i0(main.fileIni.getValue("pref", "param_stampa_" + nomeStampa + "_carattere", "0"));
        }
        ...
        try {
            //creazione del pdf
            if (cu.s(formato).equals("orizzontale")) {
                document = new Document(PageSize.A4.rotate(), 10, 10, 20, 20);
            } else {
            ...
            document.open();
            //------------------------------------
            bf = new com.lowagie.text.Font(com.lowagie.text.Font.HELVETICA, 5 + carattere, com.lowagie.text.Font.NORMAL);
            ...
            //sostituisco headerwidth con quello ricalcolato dalla griglia visualizzata
            headerWidth = getHeaderWidths(this);
...
//            datatable.setCellpadding(0);
            ...
            intestazione.add(new Chunk(titolo, new com.lowagie.text.Font(com.lowagie.text.Font.TIMES_ROMAN, 8 + carattere, com.lowagie.text.Font.BOLD)));
            if (intestazione_aggiuntiva != null) {
                intestazione.add(new Chunk("\n" + intestazione_aggiuntiva, new com.lowagie.text.Font(com.lowagie.text.Font.TIMES_ROMAN, 7 + carattere, com.lowagie.text.Font.NORMAL)));
            }
            document.add(intestazione);

            Object ret = null;
            if (pre_int_tab != null) {
                ret = pre_int_tab.call(datatable);
            }
            ...
            for (i = 0; i <= col_tutte; i++) {
                try {
                    if (headerWidth[i] > 0) {
                        ...
                        String tmp = (String) getColumnsTitlesDisplayed().get(title);
                        if (tmp != null) {
                            title = tmp;
                        }
                        tempPdfCell = new Cell(new Phrase(title, bf));
                        set1(tempPdfCell);
                    ...
                    try {
                        if (headerWidth[i] > 0) {
                            ...
                            if (process_value != null) {
                                o = process_value.call(datatable, j, i, o);
                            }
                            if (o == null) {
                                ...
                                tempPdfCell = new Cell(new Phrase(cu.s(o), bf));
                            }
                            set2(tempPdfCell);
                            datatable.addCell(tempPdfCell);
                        }
            ...
            if (piede != null) {
                ...
                pPiede.add(new Chunk(piede, new com.lowagie.text.Font(com.lowagie.text.Font.TIMES_ROMAN, 10 + carattere, com.lowagie.text.Font.BOLD)));
                document.add(pPiede);
            }
    ...
    static public void set1(Cell tempPdfCell) {
    ...
    public boolean dbOpen(Connection connection, String sql, DbI db, boolean lazy) {
        ...
        //recuper eventuali renderer e editor precedenti
        ArrayList old_renderers = memorizza_renderers();
        ArrayList old_editors = memorizza_editors();
        ...
        if (lazy == true) {
            ...
            try {
                ...
                String colonne[] = new String[numeColo];
                for (int i = 1; i <= numeColo; ++i) {
                    colonne[i - 1] = meta.getColumnLabel(i);
                }

                tm = new LazyResultSetModel(sql, db, this, colonne);
                ...
                if (this == null) {
                    ...
                    for (int i = 1; i <= numeColo; ++i) {
                        if (meta.getColumnType(i) == Types.VARCHAR
                            ...
                            getColumnModel().getColumn(i - 1).setCellRenderer(currencyRender);
                        } else if (meta.getColumnType(i) == Types.DATE) {
                            getColumnModel().getColumn(i - 1).setCellRenderer(dateRender);
...
//perchè ultima colonna rimane sempre piccola ?                    
...
//                    }
                    ...
                    //recupero renderers
                    recupera_renderers(old_renderers);
                    recupera_editors(old_editors);
            ...
            try {
                ...
                tm = new SortableTableModel(colonne, 0, meta, colonneEditabili, colonneEditabiliByName);
                //righe
                numRec = 0;
                while (resu.next()) {
                    numRec++;
                ...
                if (this == null) {
                    ...
                    final ArrayList f_old_renderers = old_renderers;
                    final ArrayList f_old_editors = old_editors;

//Commentato in EDT perchè su invia sollecito da scadenzario si blocca                     
...
//                        public void run() {
                    ...
                    //recupero renderers
                    recupera_renderers(f_old_renderers);
                    recupera_editors(f_old_editors);
...
//                    });
                ...
                SwingUtils.showExceptionMessage(getParent(), sqlex);
            } catch (Exception err) {
                err.printStackTrace();
                System.out.println("sql errore:" + sql);
            } finally {
    ...
    private void recupera_renderers(ArrayList old_renderers) {
        if (old_renderers != null) {
            if (getColumnModel() != null && getColumnCount() == old_renderers.size()) {
                for (int i = 0; i < getColumnCount(); i++) {
                    if (old_renderers.get(i) != null) {
                        getColumnModel().getColumn(i).setCellRenderer((TableCellRenderer) old_renderers.get(i));
                    }
                }
            }
        }
    ...
    private void recupera_editors(ArrayList old_editors) {
        if (old_editors != null) {
            if (getColumnModel() != null && getColumnCount() == old_editors.size()) {
                for (int i = 0; i < getColumnCount(); i++) {
                    if (old_editors.get(i) != null) {
                        getColumnModel().getColumn(i).setCellEditor((TableCellEditor) old_editors.get(i));
                    }
                }
            }
        }
...
//        header.addMouseListener(headerListener);
    ...
    synchronized public void dbRefresh() {
        refreshing = true;
        //aggancio a eventi del dbPanel se presente
        if (dbPanel != null && !giaAgganciato) {
            dbPanel.addDbListener(new DbListener() {
    ...
    public void dbSelezionaRiga() {
        ...
        if (getModel() instanceof LazyResultSetModel && ((LazyResultSetModel) getModel()).isLoading()) {
        ...
        if (this.dbPanel != null && this.getSelectedRowCount() > 0) {
            ...
            if (ObjectUtils.toString(getValueAt(this.getSelectedRow(), 0)).equals("*") && !dbPanel.dbStato.equals(tnxDbPanel.DB_LETTURA)) {
                return;
            ...
            for (int i = 0; i < dbChiave.size(); i++) {
                ...
                if (trovato == true) {
                    String valore = "";
                    try {
                        valore = cu.s(tm.getValueAt(this.getSelectedRow(), idCampo));
                    } catch (Exception e) {
                        System.out.println("tm = " + tm);
                        System.out.println("this.getSelectedRow() = " + this.getSelectedRow());
                        System.out.println("idCampo = " + idCampo);
                        e.printStackTrace();
                    }
                    temp.add(valore);
                    //debug
...
//                this_mousePressed(e);
    ...
    void this_mousePressed(MouseEvent e) {
        ...
        //1.3.3
        if (e.getClickCount() == 2 && e.getComponent() instanceof tnxDbGrid) {
    ...
    public static class GridRefreshInternalFrameOpened extends InternalFrameAdapter {
        ...
        private tnxDbGrid grid = null;

        public GridRefreshInternalFrameOpened(tnxDbGrid grid) {
            this.grid = grid;
        }

        @Override
        public void internalFrameOpened(InternalFrameEvent e) {
            grid.resizeColumnsPerc(true);
        }

    }

    //TODO da rimuovere dopo update di fine Giugno 2016s
    public void setNoTnxResize(boolean come) {
        //...
    }

    public void resizeColumnsPerc(final boolean forceResize) {
        ...
        if (columnsSizePerc != null) {
            ...
            if (colsWidth == 0) {
                JInternalFrame parent = SwingUtils.getParentJInternalFrame(tnxDbGrid.this);
                if (parent != null) {
                    parent.validate();
            ...
            if (colsWidth == 0) {
                ...
                //se è 0 è perchè ancora non è visibile
                //provo ad agganciare l'evento al padre quando diventa visibile
                //se non ce l'ha già
                JInternalFrame parent = SwingUtils.getParentJInternalFrame(tnxDbGrid.this);
                if (parent != null) {
                    if (!JUtil.hasListenerClass(parent.getInternalFrameListeners(), GridRefreshInternalFrameOpened.class)) {
                        parent.addInternalFrameListener(new GridRefreshInternalFrameOpened(this));
                    }
                }
                return;
            }

            if (getColumnCount() != columnsSizePerc.size()) {
                ...
                System.out.println("debug:griglia, numero colonne diverso:" + getColumnCount() + ":v=" + columnsSizePerc.size());
            }

            if (getColumnCount() > 0 && isResizing == false && colsWidth > 0) {
                if (oldWidth != getParent().getWidth() || forceResize == true) {
                    //debug
                    isResizing = true;
                    countResize++;
                    ...
                    for (java.util.Enumeration e = columnsSizePerc.keys(); e.hasMoreElements();) {
                        ...
                        test100 += (Double) columnsSizePerc.get(columnName);
                    }
                    System.out.println("test100 = " + test100);

                    double testwidth100 = 0;
                    for (java.util.Enumeration e = columnsSizePerc.keys(); e.hasMoreElements();) {
                        columnName = e.nextElement();
                        try {
                        ...
                        if (contaColonne >= 0) {
                            try {
                                ...
                                try {
                                    col = getColumn(String.valueOf(columnName));
                                } catch (Exception err) {
                                    col = getColumn(String.valueOf(columnNameLingua));
                                }
                                ...
                                if (colWidthPerc.doubleValue() != 0.0) {
                                    int newsize = (int) (colsWidth / 100d * colWidthPerc.doubleValue());
                                    testwidth100 += newsize;
                                    col.setPreferredWidth(newsize);
                                    col.setWidth(newsize);

                                    //                                    col.setMinWidth(newsize);
                                    //debug
                    ...
                    //controllo se ci sono colonne in modello che non hanno la dimensione
                    TableColumnModel tcm = getColumnModel();
                    for (int i = 0; i < tcm.getColumnCount(); i++) {
                        TableColumn tc = tcm.getColumn(i);
                        if (!columnsSizePerc.containsKey(tc.getIdentifier())) {
                    ...
                    if (columnsTitle != null) {
                        for (java.util.Enumeration e = columnsTitle.keys(); e.hasMoreElements();) {
                            ...
                            setColumnTitle(columnIndex.intValue(), caption);
                        }
                    }

                    //imposto ultima colonna
                    TableColumn lastcol = null;
                    int rimanente = getViewport().getWidth();
                    int totcol = 0;
                    int dimlastcol = 0;
                    for (int i = 0; i < getColumnCount(); i++) {
    ...
    public static int[] getHeaderWidths(tnxDbGrid griglia) {
        int[] w = new int[griglia.getColumnCount()];
        int total = 0;

        for (int i = 0; i < griglia.getColumnCount(); i++) {
            int colsize = griglia.getColumnModel().getColumn(i).getWidth();
            total += colsize;
        }

        for (int i = 0; i < griglia.getColumnCount(); i++) {
            int colsize = griglia.getColumnModel().getColumn(i).getWidth();
            //colsize : totale = x : 100
            w[i] = colsize * 100 / total;
            System.out.println("col i = " + i + " " + griglia.getColumnName(i) + " size = " + colsize + " w = " + w[i]);
        }

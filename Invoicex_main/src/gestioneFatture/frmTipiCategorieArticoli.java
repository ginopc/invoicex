...
public class frmTipiCategorieArticoli
    extends javax.swing.JInternalFrame {

    tnxbeans.tnxComboField comboToRefresh;

    /** Creates new form frmDati_blank */
    public frmTipiCategorieArticoli() {
        initComponents();

        //associo il panel ai dati
        this.dati.dbNomeTabella = "categorie_articoli";
...
//        this.dati.dbChiaveAutoInc = true;
        ...
        this.dati.dbOpen(Db.getConn(), "select * from categorie_articoli order by categoria", Db.INSTANCE);
        this.dati.dbRefresh();
        ...
        this.griglia.dbOpen(Db.getConn(), "select id, categoria from categorie_articoli order by categoria");
            ...
            .add(datiLayout.createSequentialGroup()
                ...
                .add(datiLayout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
                    ...
                    .add(datiLayout.createSequentialGroup()
                        .add(jLabel2211)
                        .addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
                        .add(categoria, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 352, Short.MAX_VALUE)
                        .addContainerGap())))
        );
    ...
    private void butStampaElencoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_butStampaElencoActionPerformed
        ...
        String nomeFilePdf = this.griglia.stampaTabella("Elenco categorie articoli", headerWidth);
    ...
    private void butDeleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_butDeleActionPerformed
        ...
        if (check instanceof String) {
            SwingUtils.showInfoMessage(this, "Non puoi eliminare questa categoria, risulta collegata a:\n" + check.toString());
            return;
        }
    ...
    private void butSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_butSaveActionPerformed
        if (dati.dbStato.equals(tnxDbPanel.DB_MODIFICA) && cu.toInteger0(dati.dbGetField("id")) != cu.toInteger0(id.getText())) {
            ...
            //se Boolean ok, se Stringa errori
            if (check instanceof String) {
                SwingUtils.showInfoMessage(this, "Non puoi modificare il codice di questa categoria, risulta collegata a:\n" + check.toString());
                return;
            }
        }
        this.dati.dbSave();
    ...
    private void butNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_butNewActionPerformed
        ...
        try {
            id.setText(cu.toString(cu.toInteger(DbUtils.getObject(Db.getConn(), "select IFNULL(max(id),0)+1 from categorie_articoli"))));
        } catch (Exception err) {
            err.printStackTrace();
        }        
    ...
    private Object controllodipendenze() {
        ...
        try {
            ...
            //sottocategorie
            String sql = "select id, sottocategoria from sottocategorie_articoli where id_padre = " + cat;
            List<Map> ret = DbUtils.getListMap(Db.getConn(), sql);
            if (ret != null && ret.size() > 0) {
                for (int i = 0; i < Math.min(ret.size(), 3); i++) {
                    Map m = ret.get(i);
                    msg += "Sottocategoria " + m.get("id") + " - " + m.get("sottocategoria") + "\n";
                }
                if (ret.size() > 3) {
                    msg += "... e altre sottocategorie\n";
                }
            }
            
            //articoli
            sql = "select codice, descrizione from articoli where categoria = " + cat;
            ret = DbUtils.getListMap(Db.getConn(), sql);
            if (ret != null && ret.size() > 0) {
                for (int i = 0; i < Math.min(ret.size(), 3); i++) {
                    Map m = ret.get(i);

    ...
    static private void init() {
        if (c1 == null) {
            try {
                ...
                mb.out("iniFileProp init 1");
                d1 = Cipher.getInstance("DES");
                mb.out("iniFileProp init 2");
                secretKey = new SecretKeySpec(desKeyData, "DES");
                mb.out("iniFileProp init 3");
                c1.init(Cipher.ENCRYPT_MODE, secretKey);
                mb.out("iniFileProp init 4");
                d1.init(Cipher.DECRYPT_MODE, secretKey);
                mb.out("iniFileProp init 5");
            } catch (Exception err) {
                err.printStackTrace();
            }
        }
...
//    public synchronized String getValueCifrato(String subject, String variable) {
    ...
    public String getValueCifrato(String subject, String variable) {
        return getValueCifrato(subject, variable, null);
    }
    public String getValueCifrato(String subject, String variable, String def) {
        ...
        try {
            ...
            if (def != null && (temp4 == null || temp4.trim().length() == 0)) {
                return def;
            }
...
//    public synchronized String getValue(String subject, String variable) {
    ...
    static public String crypt(String inchiaro) {
        init();
        try {
            byte[] temp1 = c1.doFinal(inchiaro.getBytes());
            byte[] temp2 = Base64.encodeBase64(temp1);
            String temp3 = new String(temp2);            
            return temp3;
        } catch (Exception e) {
    ...
    static public String decrypt(String criptata) {
        init();
        try {
            byte[] temp2 = Base64.decodeBase64(criptata.getBytes());
            byte[] temp3 = d1.doFinal(temp2);
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        System.out.println(crypt("dsjklf8934c93ufn2"));
        System.out.println(decrypt("HXy/KFhzwzZI2WPiSuHd+A\\=\\="));
        System.out.println(decrypt("6mnxREKZAbQdmGdtBYZk9Q\\=\\="));
    }
}
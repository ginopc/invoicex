...
public class frmArtiComposti extends javax.swing.JInternalFrame {
    ...
    public frmArtiComposti(String codice) {
        ...
        griglia.getColumn(0).setPreferredWidth(100);
        ...
        //id
//        griglia.getColumn(4).setPreferredWidth(0);
//        griglia.getColumn(4).setMaxWidth(0);
//        griglia.getColumn(4).setMinWidth(0);
//        griglia.getColumn(4).setWidth(0);
        griglia.getColumnModel().removeColumn(griglia.getColumn(4));

        //renderer e editors
//        JPanelCellKeyValue panel_articolo = new JPanelCellKeyValue();
//        panel_articolo.campo_descrizione = "descrizione";
//        panel_articolo.campo_id = "codice";
//        panel_articolo.tabella = "articoli";
//        griglia.getColumn("Codice").setCellRenderer(new TableCellRendererKeyValue(griglia, panel_articolo));
//        JPanelCellKeyValue panel_articolo_editor = new JPanelCellKeyValue();
//        panel_articolo_editor.campo_descrizione = "descrizione";
//        panel_articolo_editor.campo_id = "codice";
//        panel_articolo_editor.tabella = "articoli";
        al_articolo = InvoicexUtil.getArticoloIntelliHints(articolo, this, articolo_selezionato_ref, null, null);
        al_articolo.addPropertyChangeListener(new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                try {
                    articolo.setText(articolo_selezionato_ref.get().codice);
                } catch (Exception e) {
                }
            }
        });
        griglia.getColumn("Codice").setCellEditor(new DefaultCellEditor(articolo));

        griglia.getColumn("Quantità").setCellEditor(new EditorUtils.NumberEditor(new JTextField(), new DecimalFormat("0.#####")));
        griglia.getColumn("Quantità").setCellRenderer(new RendererUtils.NumberRenderer(0, 5));
        ...
        try {
            ...
            if (rs.next()) {
                ...
                sql = "select p.articolo, p.quantita, a.um, a.descrizione " + campoid + " from pacchetti_articoli p join articoli a on p.articolo = a.codice where p.pacchetto = '" + codice + "'";
    ...
    private void recuperaDatiArticolo(String chiave, int row) {
        if (chiave != null && chiave.length() > 0) {
            ...
            if (texCodicePacchetto.getText().equals(chiave)) {
...
//                        javax.swing.JOptionPane.showMessageDialog(this, "Il codice articolo specificato non esiste in anagrafica !");
    ...
    private void saveKit() {
        ...
        //controllare che non ci siano kit o se stesso dentro al kit
        //TODO
    ...
    private void removeBlackRow() {
        for (int row = 0; row < griglia.getRowCount(); row++) {
            ...
            for (int col = 0; col < griglia.getColumnCount(); col++) {
                if (!cu.s(griglia.getValueAt(row, col)).equals("")) {
                    delete = false;
                }
            }
            if (delete) {
    ...
    private void initComponents() {
        ...
        texCodicePacchetto.setEditable(false);

        jLabel2.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
        jLabel2.setText("Descrizione:"); // NOI18N

        texDescrizionePacchetto.setEditable(false);
        ...
        griglia.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Codice", "Descrizione", "Quantità", "Unità di Misura", "id"
            }
        ) {
        ...
        if (griglia.getColumnModel().getColumnCount() > 0) {
            griglia.getColumnModel().getColumn(4).setResizable(false);
            griglia.getColumnModel().getColumn(4).setPreferredWidth(0);
        }
    ...
    private void saveKitSync() {
        ...
        //elimino solo record non più presenti tramite confronto con list_prezzi_pre
        try {
            HashMap list_post = syncGetList();
//            for (Object k : list_pre.keySet()) {
//                System.out.println("k:" + k);
//                System.out.println("list_pre:" + list_pre.get(k));
//                System.out.println("list_post:" + list_post.get(k));
//            }
            List<Map> diffs = Sync.getDifferences(list_pre, list_post);
            System.out.println("diffs = " + diffs);
            Sync.execDifferences(Db.getConn(), diffs, "pacchetti_articoli");
    ...
    private HashMap syncGetList() throws Exception {
        ...
        tras.put("Codice", "articolo");
        tras.put("Quantità", "quantita");
        Map fixed = new HashMap();
        fixed.put("pacchetto", texCodicePacchetto.getText());
        return DbUtils.getListMapMapFromTableModel(griglia.getModel(), "id", new String[]{"Codice", "Quantità", "id"}, null, tras, fixed);
    }
}

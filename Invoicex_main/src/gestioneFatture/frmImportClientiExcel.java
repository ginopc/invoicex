...
//                            String[] colonne = new String[] {"ragione_sociale", "indirizzo", "cap", "localita", "provincia", "cfiscale", "piva_cfiscale", "telefono", "fax"};
                            String[] colonne = new String[]{"ragione_sociale", "indirizzo", "cap", "localita", "provincia", "cfiscale", "piva_cfiscale", "telefono", "fax", "cellulare", "agente", "email", "web", "persona_riferimento", "telefono_riferimento", "note", "pagamento", "codice_listino", "iva_standard", "banca_abi", "banca_cab", "banca_cc", "banca_cc_iban", "sconto1t", "sconto2t", "sconto3t", "sconto1r", "sconto2r", "tipo", "paese", "tipo2"};
                            for (int i = 0; i <= sheet.getLastRowNum(); i++) {
                                ...
                                try {
                                    ...
                                    if (!StringUtils.isEmpty(cfiscale)) {
                                        ...
                                        if (codice != null) {
                                            System.out.println("TROVATO codice1:" + codice + " sql:" + sql);
                                        }
                                    }
                                    if (codice == null && !StringUtils.isEmpty(piva)) {
                                        ...
                                        codice = DbUtils.getString(Db.getConn(), sql);
                                        if (codice != null) {
                                            System.out.println("TROVATO codice2:" + codice + " sql:" + sql);
                                        }
                                    }
                                    if (codice == null && !StringUtils.isEmpty(rs)) {
                                        ...
                                        codice = DbUtils.getString(Db.getConn(), sql);
                                        if (codice != null) {
                                            System.out.println("TROVATO codice3:" + codice + " sql:" + sql);
                                        }
                                    }
...
//                                        }
                                    ...
                                    try {
                                        Integer id_autoinc = null;
                                        if (dbu.getObject(Db.getConn(), "select codice from clie_forn where codice = " + dbu.sql(codice), false) != null) {
                                            //update
                                            ...
                                            //insert
                                            id_autoinc = Db.executeSqlRetIdDialogExc(Db.getConn(), sql, false, false);
                                            contaok++;
                                        }

                                        if (recxls.containsKey("persona_riferimento") && StringUtils.isNotBlank(cu.s(recxls.get("persona_riferimento")))) {
                                            //cerco contatto per nome, se non lo trovo lo inserisco altrimenti lo modifico
                                            //telefono_riferimento
                                            if (dbu.getObject(Db.getConn(), "select id from clie_forn_contatti where id_clifor = " + dbu.sql(codice) + " and nome like " + dbu.sql(cu.s(recxls.get("persona_riferimento"))), false) != null) {
                                                //modifica
                                                if (recxls.containsKey("telefono_riferimento") && StringUtils.isNotBlank(cu.s(recxls.get("telefono_riferimento")))) {
                                                    sql = "update clie_forn_contatti set telefono = " + dbu.sql(cu.s(recxls.get("telefono_riferimento")))
                                                            + " where id_clifor = " + dbu.sql(codice) + " and nome like " + dbu.sql(cu.s(recxls.get("persona_riferimento")));
                                                    try {
                                                        DbUtils.tryExecQuery(Db.getConn(), sql);
                                                    } catch (Exception e) {
                                                        e.printStackTrace();
                                                ...
                                                //ins
                                                sql = "insert into clie_forn_contatti "
                                                        + " set id_clifor = " + dbu.sql((id_autoinc == null ? codice : id_autoinc))
                                                        + " , telefono = " + dbu.sql(cu.s(recxls.get("telefono_riferimento")))
                                                        + " , nome = " + dbu.sql(cu.s(recxls.get("persona_riferimento")));
                                                try {
                                                    DbUtils.tryExecQuery(Db.getConn(), sql);
                                                } catch (Exception e) {
                                                    e.printStackTrace();
